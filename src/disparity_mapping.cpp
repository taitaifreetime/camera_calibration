...
int main(int argc, char* argv[])
{
    ...
    int left_camera_fps = std::atoi(argv[5]);
    int right_camera_fps = std::atoi(argv[6]);
    ...
    if (
        ...
        left_width == 0 || left_height == 0 ||  
        left_camera_matrix.empty() ||
        left_dist_coeffs.empty() ||
        left_rectification_matrix.empty() ||
        left_projection_matrix.empty() ||
        // right camera parameter
        right_width == 0 || right_height == 0 ||  
        right_camera_matrix.empty() ||
        right_dist_coeffs.empty() ||
        right_rectification_matrix.empty() ||
        right_projection_matrix.empty()
    )
    {
        std::cerr << "Some parameters not found" << std::endl;
        return 1; 
    }
    ...
    cv::initUndistortRectifyMap(
        ...
        left_rectification_matrix, left_projection_matrix, cv::Size(left_width, left_height),
        CV_32FC1, map1x, map1y);
    cv::initUndistortRectifyMap(
        right_camera_matrix, right_dist_coeffs, 
        right_rectification_matrix, right_projection_matrix, cv::Size(right_width, right_height),
    ...
    cv::VideoCapture capL(utils::capture::getGstreamer(left_camera_id, left_width, left_height, left_camera_fps, 0), cv::CAP_GSTREAMER);
    cv::VideoCapture capR(utils::capture::getGstreamer(right_camera_id, right_width, right_height, right_camera_fps, 0), cv::CAP_GSTREAMER);
    ...
    cv::Mat rectL, rectR, disparity, disp8;
    ...
    while(true) {
        ...
        // --- Rectify images ---
        cv::remap(frameL, rectL, map1x, map1y, cv::INTER_LINEAR);
        cv::remap(frameR, rectR, map2x, map2y, cv::INTER_LINEAR);

        // --- Convert to grayscale ---
        cv::cvtColor(rectL, grayL, cv::COLOR_BGR2GRAY);
        cv::cvtColor(rectR, grayR, cv::COLOR_BGR2GRAY);
        cv::resize(grayL, grayL, cv::Size(), 0.5, 0.5);
        cv::resize(grayR, grayR, cv::Size(), 0.5, 0.5);

        // --- Compute disparity ---
        sgbm->compute(grayL, grayR, disparity);
        disparity.convertTo(disp8, CV_8U, depth_scale);
        ...
        cv::imshow("Right Rectified", rectR);
#endif
